...
//  FruitMart
//
//  Created by Woogie on 2022/02/04.
...
struct ProductDetailView: View {
    ...
    var orderView: some View {
        GeometryReader {
            VStack(alignment: .leading) {
                self.productDescreiption // 상품명, 즐겨찾기 버튼(하트모양) 이미지
                Spacer()
                self.priceInfo   // 가격 정보
                self.placeOrderButton   // 주문하기 버튼
                Spacer()
            }
            // 지오메트리 리더 뷰의 높이보다 VStack의 높이가 10 크도록 지정
    ...
    // 상품명, 즐겨찾기 버튼, 상품설명
    var productDescreiption: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text(splitText(product.description)) //상품 설명 , splitText == 한 문장을 반으로 나눠 두 줄로 나누어줌
